...
//					fprintf(fp_files,"%s\n",tmppath);
			...
			for (;p1 != NULL;) {
				...
				//param.hiEQAdjust = 1;
				if (strcmp(p1,"-hiEQAdjust") == 0) {
					param.hiEQAdjust = 1;
					param.Adjust_enable = 1;
					PRINT_LOG("Adjust_enable");
				}
 ...
 */
{
	...
	do {
		...
		if (param->eq_pw != NULL) {
			...
			param->eq_pwcount = 32;
			if (param->hiEQAdjust == 1) {
				param->hiEQAdjust_mode = 1;
			}
		}
		PRINT_LOG("1");
		...
		if (param->hiEQAdjust == 1) {
			int i,j;
			double avg,p1_avg,p2_avg,p1_step,p1_per,p2_per;
			int p1_index,p2_index,p1_e_index,p2_e_index;

			validIndex = (((double)param->outSampleR * 2) / param->outSampleR) * (param->inSampleR / 2);

			for (i = 0;i < validIndex + 15;i++) {
				avg = 0;
				for (j = 0;j < 15;j++) {
					avg += param->eq_pw[i + j];
				}
			...
			p1_index = ((double)(param->inSampleR * 2) / param->inSampleR) * 4000;
			p1_e_index = ((double)(param->inSampleR * 2) / param->inSampleR) * 5000;
			p2_index = ((double)(param->inSampleR * 2) / param->inSampleR) * 8000;
			p2_e_index = ((double)(param->inSampleR * 2) / param->inSampleR) * 11000;
			p1_avg = 0;
			p2_avg = 0;
			for (i = p1_index;i < p1_e_index;i++) {
				p1_avg += param->eq_pw[i];
			}
			if (i > 0) {
				p1_avg /= i;
			}
			for (i = p2_index;i < p2_e_index;i++) {
				p2_avg += param->eq_pw[i];
			}
			if (i > 0) {
				p2_avg /= i;
			}
			...
			if (p1_per > 0.6) {
				...
				for (i = 1;i < validIndex;i++) {
					if (i > p2_index) {
						param->eq_pw[i] = param->eq_pw[i - 1];
					} else if (i > p1_index) {
						param->eq_pw[i] = param->eq_pw[i - 1] - p1_step;
					} else {
						param->eq_pw[i] = 1;
					}
				}
				for (i = 1;i < validIndex;i++) {
					if (i > p1_index && param->eq_pw[i] < 0.1) {
						param->eq_pw[i] = 0.1;
					}
				}
			} else {
			...
			param->hiEQAdjust_mode = 2;
		}
		if (param->hiEQAdjust == 0) {
			param->hiEQAdjust_mode = 0;
		}
		...
		// fft
		param->enable_filter = 1;
		fftFilter(inSample,outSample,&fp_r,&fp_w,param);
		param->enable_filter = 0;
		param->hiEQAdjust_mode = 0;
		if (param->eq_pw != NULL) free(param->eq_pw);
		param->eq_pw = NULL;
		if (param->err) {
			PRINT_LOG("ERROR");
			break;
...
#if 1
	if (param->Adjust_enable == 1) {
		if (id == 1 && param->hiEQAdjust == 1 && param->eq_pw != NULL && param->hiEQAdjust_mode == 1) {
			PRINT_LOG("bb");
			if (param->hiEQAdjust == 1) {
				PRINT_LOG("bb1");
			}
			if (param->hiEQAdjust_mode == 1) {
				PRINT_LOG("bb2");
			}
			char sssss[128];
			sprintf(sssss,"validIndex=%d",validIndex);
			PRINT_LOG(sssss);
			for (i = 1;i < validIndex;i++) {
...
#endif
	if (param->enable_filter) {
#if 1
		if (param->Adjust_enable == 1) {
			if (param->hiEQAdjust == 1 && param->eq_pw != NULL && param->hiEQAdjust_mode == 2) {
				PRINT_LOG("cc");
				for (i = 1;i < validIndex;i++) {
					fftw_out[i][0] *= param->eq_pw[i];
					fftw_out[i][1] *= param->eq_pw[i];
				}
			}
		}
#endif
...
#if 0
				if (param->validOutSampleR <= 96000) {
					startIdx = ((double)fftSizeOut / outSampleR) * 46000;
					endIdx	 = ((double)fftSizeOut / outSampleR) * 78000;
					step = 1.00 / ((endIdx - startIdx) * 1.71);
				} else {
					startIdx = ((double)fftSizeOut / outSampleR) * 56000;
					endIdx	 = ((double)fftSizeOut / outSampleR) * 95000;
