...
PRINT_LOG("store:all");
	...
	fio_r->block_max = 5;
	fio_r->block_count = 5;
	fio_r->ibuffer_size = fio_w->ibuffer_size;
	if (fio_r->ibuffer != NULL) free(fio_r->ibuffer);
	fio_r->ibuffer = malloc(fio_r->ibuffer_size * 1000 * 1000L);
	if (fio_r->ibuffer == NULL) {
		fio_r->error = FIO_ERR_MEMORY;
		PRINT_LOG("ERROR");
		return;
	}
...
void fio_set_memory_limit(FIO *fio,int block_sz,int max)
{
	...
	// check magic no
	if (fio->magic != 5915 || max < 0 || max > P_BUF_SIZE_N || block_sz < 1 || block_sz > 100) {
		fio->error = FIO_ERR_OTHER;
		PRINT_LOG("ERROR");
		return;
	}
	...
	if (fio->ibuffer != NULL) free(fio->ibuffer);
	fio->ibuffer = malloc(fio->ibuffer_size * 1000 * 1000L);
	if (fio->ibuffer == NULL) {
		fio->error = FIO_ERR_MEMORY;
		PRINT_LOG("ERROR");
		return;
...
void fio_copy_file(FIO *fio_r,FILE *ofp)
{
	...
	while (remain) {
		if (remain >= fio_r->ibuffer_size * 1024 * 1024L) {
			n = fread(fio_r->ibuffer,fio_r->ibuffer_size * 1024 * 1024L,1,fio_r->fp);
		} else {
		...
		if (remain >= fio_r->ibuffer_size * 1024 * 1024L) {
			n = fwrite(fio_r->ibuffer,fio_r->ibuffer_size * 1024 * 1024L,1,ofp);
			remain -= fio_r->ibuffer_size * 1024 * 1024L;
		} else {
